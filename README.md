...
Everything the skill needs *to operate* lives inside `skill/` itself — data and scripts included, not as repo-root siblings — so copying or symlinking that one folder anywhere gives a fully self-contained, portable skill; nothing has to be separately carried along by hand. That's a narrower claim than "a clean clone can regenerate everything from scratch," though: the script that produces `skill/data/riftcodex_cards_raw.json` in the first place isn't in this repo (see [`skill/data/README.md`](skill/data/README.md)'s Regenerating section) — operating self-contained and being fully reproducible from zero are two different properties, and this repo only has the first one. `skill/` is a small library, not one giant prompt: a thin `SKILL.md` router plus two independent books —
