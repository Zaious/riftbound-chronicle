...
FEATURE_RULES = {
    ...
    # C-33 (ADR-0009 §1–2, §5–7).
    "battlefield_control_resolution": ["Core 190.4", "Core 466.5–466.5.e", "Core 348.2.a–348.2.a.1"],
...
KIND_CONFIG = {
    ...
    "effect": {
        ...
        "supported": ["typed_atomic_effects", "bounded_replacement", "bounded_cleanup", "typed_selectors", "object_identity", "engine_decisions", "battlefield_targets", "criteria_expansion", "bonus_damage", "instruction_conditions", "move_triggers", "private_discard", "granted_replacements", "combat_relative_might", "keyword_modifiers", "active_combat_criteria", "mutual_current_might_damage", "burn_out_draw", "randomization_receipt", "choice_grammar", "modal_abilities", "card_ordering", "look_reveal_put_back", "predict", "multi_recycle", "restricted_resource_grant", "repeat_execution", "banish", "counter", "burn", "attachments", "top_most_might_bonus", "derived_detach_destination", "legend_objects", "legend_zone_passives", "continuous_effects", "layer_engine", "canonical_migration", "typed_conditions", "keyword_catalog", "empower_state", "buff_counters", "xp", "effect_text_append", "copy_modelled_traits"],
        "unsupported": ["arbitrary_card_text", "combat", "scoring", "burn_out_non_draw", "burn_out_in_replacement", "distinct_per_turn_choice", "each_player_choice", "multi_opponent_choice", "revealed_knowledge_persistence", "cannot_be_countered", "burn_triggers", "nested_attachments", "gear_recall_cleanup", "copy_unmodelled_traits", "champion_legend_movement", "token_not_in_catalogue", "layer_dependency_unresolved", "condition_needs_hidden_information", "keyword_not_implemented", "complete_game", "complete_legality"],
    },
    "resolution": {
    ...
    "turn_step": {
        ...
        "unsupported": ["setup_procedure", "match_mode", "team_scoring", "ready_blockers", "opponent_phase_watchers", "continuous_dependency", "facedown_reveal_at_game_end", "complete_game", "complete_legality"],
    },
...
def run_control_step(args: argparse.Namespace) -> tuple[dict[str, Any], dict[str, str]]:
    timing_state, effect_state = load_object(args.timing_state), load_object(args.effect_state)
    decisions = _engine_decisions(getattr(args, "decisions", None))
    result = CONTROL_STEPS[args.step](timing_state, effect_state, decisions)
    hashes = {"timing_state": state_hash(timing_state), "effect_state": hash_value(effect_state)}
    if decisions is not None:
        hashes["engine_decisions"] = canonical_hash(decisions)
    return result, hashes
...
def build_parser() -> argparse.ArgumentParser:
    ...
    control = sub.add_parser("control-step")
    control.add_argument("timing_state", type=Path)
    control.add_argument("effect_state", type=Path)
    control.add_argument("--step", choices=sorted(CONTROL_STEPS), required=True)
    add_common(control)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "validate":
            ...
            return 0
        runner = {"timing": run_timing, "effect": run_effect, "resolution": run_resolution, "cleanup": run_cleanup, "play": run_play, "turn-step": run_turn_step, "combat-step": run_combat_step, "standard-move": run_standard_move, "control-step": run_control_step}[args.command]
        kind = {"turn-step": "turn_step", "combat-step": "combat_step", "standard-move": "standard_move", "control-step": "control_step"}.get(args.command, args.command)
